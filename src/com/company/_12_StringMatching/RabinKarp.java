...
public class RabinKarp {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i <= str.length() - pattern.length(); i++) {
            if(patHash == strHash &&
            checkEqual(str, pattern, i,i+pattern.length()-1,
                    0,pattern.length()-1)){
                System.out.print(i + " ");
            }
            if(i < str.length()-pattern.length()){
                strHash = recalculateHash(str,i,i+pattern.length(),
                        strHash,pattern.length());
            }
        }
    }
    ...
    public static long createHashValue(String str, int n){
        long result = 0;
        for (int i = 0; i < n; i++) {
            result += str.charAt(i)*(long)Math.pow(prime, i);
        }
        return result;
    }

    public static long recalculateHash(String str, int oldInd,
                                       int newInd, long oldHash, int patlength){
        long newHash = oldHash - str.charAt(oldInd);
        newHash /= prime;
        newHash += str.charAt(newInd)*(long)(Math.pow(prime,patlength-1));
        return newHash;
    }

    public static boolean checkEqual(String s1, String s2, int start1,
